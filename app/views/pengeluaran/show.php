...
<div class="max-w-3xl">
    <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-5">
        <div class="flex items-start justify-between gap-3 mb-4">
            ...
            <a href="<?= url('pengeluaran') ?>" class="btn btn-ghost">Kembali</a>
        </div>
        <?php if ($isBatal): ?>
        ...
        <div class="grid grid-cols-2 gap-3 text-sm mb-4">
            ...
            <div><span class="text-slate-500 block text-xs">Nominal</span><b class="text-red-600"><?= rupiah($tx['total']) ?></b></div>
        ...
        <div class="border border-slate-100 rounded-lg px-4 py-3 text-sm mb-4">
            <div class="flex justify-between">
                <span class="text-slate-500">Kas berkurang</span>
                <b class="text-red-600"><?= !empty($cash) ? rupiah($cash[0]['nominal']) : '-'; ?></b>
            </div>
        </div>
        ...
        <div class="flex items-start justify-between gap-4 mb-4">
            ...
            <?php if (!$isBatal): ?>
                <form method="post" action="<?= url('pengeluaran', ['action' => 'upload', 'id' => $tx['id']]) ?>" enctype="multipart/form-data" class="flex items-center gap-2">
                    <?= csrf_field() ?>
        ...
        <?php if (has_role('Administrator') && !$isBatal): ?>
            <div class="bg-red-50 rounded-xl border border-red-200 p-4 mt-2">
                ...
                <form method="post" action="<?= url('pengeluaran', ['action' => 'cancel', 'id' => $tx['id']]) ?>">
                    <?= csrf_field() ?>
                    <div class="flex gap-2">
                        <input type="text" name="alasan" class="input" placeholder="Alasan pembatalan (wajib)" required>
                        <button type="submit" class="btn btn-danger whitespace-nowrap" onclick="return appConfirmSubmit(event, 'Batalkan pengeluaran ini? Kas akan dibalik.')"><?= icon('x', 'w-4 h-4') ?> Batalkan</button>
                    </div>
                </form>
            </div>
        <?php endif; ?>
