<?php
list($statusLabel, $statusCls) = status_tagihan((float)$hutang['total'] - (float)$sisa, (float)$sisa);
$dibayar = (float)$hutang['total'] - (float)$sisa;
?>
<div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
    <div class="lg:col-span-2 space-y-4">
        <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-5">
            <div class="flex items-start justify-between gap-3 mb-4">
                <div>
                    <h2 class="font-semibold text-slate-800 text-lg"><?= e($hutang['supplier']) ?></h2>
                    <p class="text-sm text-slate-500"><?= e($hutang['no_transaksi']) ?> &middot; <?= tanggal($hutang['tanggal']) ?></p>
                </div>
            ...
            <div class="grid grid-cols-3 gap-3 text-center mb-4">
                <div class="bg-slate-50 rounded-lg p-3">
                    <div class="text-xs text-slate-500">Total Hutang</div>
                    <div class="text-lg font-bold"><?= rupiah($hutang['total']) ?></div>
                </div>
                ...
                <div class="bg-orange-50 rounded-lg p-3">
                    ...
                    <div class="text-lg font-bold text-orange-600"><?= rupiah($sisa) ?></div>
                </div>
            </div>

            <?php if ($hutang['status'] === 'DIBATALKAN'): ?>
                <div class="mb-4 text-xs text-red-600">Hutang ini telah dibatalkan (pembelian dibatalkan).</div>
            <?php endif; ?>
        </div>
                ...
                <table>
                    ...
                    <?php else: foreach ($payments as $p): ?>
                        <tr class="<?= $p['status'] === 'DIBATALKAN' ? 'opacity-50' : '' ?>">
                            ...
                            <td class="text-right font-semibold text-red-600"><?= rupiah($p['nominal']) ?></td>
                            <td><?= e($p['keterangan'] ?: '-') ?></td>
                            <td class="text-xs text-slate-500"><?= e($p['username'] ?? '-') ?></td>
                            <td>
                                <?php if (has_role('Administrator') && $p['status'] === 'AKTIF'): ?>
                                    <form method="post" action="<?= url('hutang', ['action' => 'cancel_bayar', 'id' => $p['id']]) ?>" class="inline">
                                        <?= csrf_field() ?>
    ...
    <div class="space-y-4">
        <?php if ($sisa > 0 && $hutang['status'] === 'AKTIF') : ?>
        <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-5">
            <h3 class="font-semibold text-slate-800 mb-3">Catat Pembayaran</h3>
            <form method="post" action="<?= url('hutang', ['action' => 'bayar', 'id' => $hutang['id']]) ?>" class="space-y-3">
                <?= csrf_field() ?>
                <div>
                ...
                <button type="submit" class="btn btn-primary w-full"><?= icon('bank', 'w-4 h-4') ?> Bayar Hutang</button>
            </form>
        </div>
        <?php endif; ?>

        <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-5">
            <a href="<?= url('hutang') ?>" class="btn btn-secondary w-full">Kembali</a>
        </div>
    </div>
</div>