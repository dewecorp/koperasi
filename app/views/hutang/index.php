<?php
$hutangUpdateTpl = url('hutang', ['action' => 'update', 'id' => '__ID__']);
?>
    ...
    <div class="table-wrap">
        <table>
            ...
            <?php else: foreach ($pg['items'] as $r): list($label, $cls) = status_tagihan((float)$r['dibayar'], (float)$r['sisa']); ?>
                <?php $htRowJson = htmlspecialchars(json_encode(['id' => $r['id'], 'no_transaksi' => $r['no_transaksi'], 'supplier' => $r['supplier'], 'total' => $r['total'], 'jatuh_tempo' => $r['jatuh_tempo']], JSON_UNESCAPED_UNICODE), ENT_QUOTES, 'UTF-8'); ?>
                <tr>
                    ...
                    <td>
                        <div class="flex items-center justify-center gap-1">
                            ...
                            <?php if (has_role('Administrator') || has_role('Bendahara')): ?>
                                <button type="button" onclick="openHutangEditModal(JSON.parse(this.dataset.row))" data-row="<?= $htRowJson ?>" class="btn btn-ghost p-1.5" title="Ubah"><?= icon('edit', 'w-4 h-4') ?></button>
                            <?php endif; ?>
...
<div id="hutangEditModal" class="hidden fixed inset-0 z-50 items-center justify-center p-4">
    <div class="absolute inset-0 bg-black/50" onclick="closeHutangEditModal()"></div>
    <div class="relative bg-white rounded-xl shadow-xl w-full max-w-md max-h-[90vh] overflow-y-auto">
        <div class="sticky top-0 bg-white border-b border-slate-200 px-6 py-4 flex items-center justify-between rounded-t-xl">
            <div>
                <h3 class="font-semibold text-slate-800">Ubah Hutang</h3>
                <p id="hutangEditModalInfo" class="text-xs text-slate-500"></p>
            </div>
            <button type="button" onclick="closeHutangEditModal()" class="btn btn-ghost p-2"><?= icon('x', 'w-5 h-5') ?></button>
        </div>
        <form method="post" id="formHutangEditModal" action="<?= $hutangUpdateTpl ?>" class="grid grid-cols-1 gap-4 p-6">
            <?= csrf_field() ?>
            <div>
                <label class="label">Total Hutang</label>
                <input type="text" id="hutang_edit_total" class="input" disabled>
            </div>
            <div>
                <label class="label">Jatuh Tempo</label>
                <input type="date" name="jatuh_tempo" id="hutang_edit_jatuh_tempo" class="input">
            </div>
            <div class="flex justify-end gap-2 pt-2">
                <button type="button" onclick="closeHutangEditModal()" class="btn btn-secondary">Batal</button>
...
<script>
var _hutangUpdateTpl = <?= json_encode($hutangUpdateTpl) ?>;
function openHutangEditModal(data){
    var modal=document.getElementById('hutangEditModal');
    var form=document.getElementById('formHutangEditModal');
    if(!data||!modal||!form) return;
    form.action=_hutangUpdateTpl.replace('__ID__', data.id);
    var info=document.getElementById('hutangEditModalInfo');
    if(info) info.textContent=(data.no_transaksi||'')+' — '+(data.supplier||'');
    var totalEl=document.getElementById('hutang_edit_total');
    if(totalEl){
        var v=(data.total||'').toString().replace(/[^0-9]/g,'');
        totalEl.value=v ? Number(v).toLocaleString('id-ID') : (data.total||'');
    }
    var jt=document.getElementById('hutang_edit_jatuh_tempo');
...
function closeHutangEditModal(){
    var m=document.getElementById('hutangEditModal');
    if(!m) return;
    m.classList.add('hidden');
    m.classList.remove('flex');
    document.body.style.overflow='';
}
document.addEventListener('keydown', function(e){ if(e.key==='Escape') closeHutangEditModal(); });
</script>
