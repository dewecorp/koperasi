...
<div class="bg-white rounded-xl shadow-sm border border-slate-200 p-5 mb-6">
    <div class="flex flex-col sm:flex-row sm:items-center gap-3 justify-between mb-4">
        ...
        <div class="text-sm font-medium">
            Total periode: <b class="text-emerald-600"><?= rupiah(array_sum(array_column($pg['items'], 'total'))) ?></b>
        </div>
    </div>
...
<div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
    <h2 class="font-semibold text-slate-800 mb-4">Catat Pemasukan Lain</h2>
    <form method="post" action="<?= url('pemasukan', ['action' => 'store']) ?>" enctype="multipart/form-data" class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <?= csrf_field() ?>
        <div>
        ...
        <div>
            ...
            <input type="text" name="sumber" class="input" placeholder="Contoh: SPP, jasa fotokopi..." value="<?= e(old('sumber')) ?>">
        </div>
        <div>
        ...
        <div class="sm:col-span-2 flex justify-end pt-2">
            <button type="submit" class="btn btn-primary"><?= icon('check', 'w-4 h-4') ?> Simpan Pemasukan</button>
        </div>
    </form>
</div>