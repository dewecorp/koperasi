...
$produkData = [];
foreach ($produk as $p) $produkData[] = ['id'=>(int)$p['id'],'nama'=>$p['name'],'kode'=>$p['kode'],'barcode'=>$p['barcode']??'','harga'=>(float)$p['harga_jual'],'stok'=>(float)$p['stock'],'satuan'=>$p['satuan']];
?>
<div class="w-full">
    <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
        <div class="flex items-center justify-between mb-5">
            <div>
                ...
                <p class="text-sm text-slate-500"><?= e($tx['no_transaksi']) ?> — <?= e($tx['pelanggan'] ?? 'Umum') ?> · <?= tanggal($tx['tanggal']) ?></p>
            </div>
            <a href="<?= url('penjualan', ['action' => 'show', 'id' => $tx['id']]) ?>" class="btn btn-ghost"><?= icon('chevron-left', 'w-4 h-4') ?> Kembali</a>
        </div>

        <?php if ($isBatal): ?>
            <div class="mb-4 px-4 py-3 rounded-lg bg-red-50 border border-red-300 text-red-700 text-sm">Transaksi dibatalkan — tidak dapat diubah.</div>
        <?php endif; ?>

        <form method="post" action="<?= url('penjualan', ['action' => 'update', 'id' => $tx['id']]) ?>" id="formPenjualanEdit" class="space-y-6">
            <?= csrf_field() ?>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-4 gap-4">
                ...
                <div class="sm:col-span-2">
                    ...
                    <select name="customer_id" class="input" id="selectCustomer" <?= $isBatal ? 'disabled' : '' ?>>
                        <option value="">- Umum / Tunai -</option>
                        <?php foreach ($pelanggan as $p): ?>
                            <option value="<?= $p['id'] ?>" <?= (string)($tx['customer_id'] ?? '') === (string)$p['id'] ? 'selected' : '' ?>><?= e($p['name']) ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                <div>
                    <label class="label">Metode Pembayaran *</label>
                    <select name="metode" class="input" id="selectMetode" <?= $isBatal ? 'disabled' : '' ?>>
                        <option value="tunai" <?= $tx['payment_method']==='tunai'?'selected':'' ?>>Tunai</option>
                        <option value="kredit" <?= $tx['payment_method']==='kredit'?'selected':'' ?>>Kredit</option>
                    </select>
                </div>
            </div>

            <div class="flex items-center gap-2 bg-slate-50 border border-emerald-200 rounded-xl px-4 py-3 <?= $isBatal ? 'opacity-50 pointer-events-none' : '' ?>">
                <?= icon('printer', 'w-5 h-5 text-emerald-600') ?>
                <input type="text" id="inputScan" class="input flex-1" placeholder="Scan barcode barang untuk tambah cepat..." autocomplete="off" <?= $isBatal ? 'disabled' : '' ?>>
                <span class="text-xs text-slate-400 hidden sm:inline">Tekan Enter / gunakan scanner</span>
            </div>

            <div class="border border-slate-200 rounded-xl overflow-hidden <?= $isBatal ? 'opacity-60' : '' ?>">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 items-end">
                <div class="sm:col-span-2">
                    ...
                    <input type="text" name="keterangan" class="input" value="<?= e($tx['keterangan']) ?>" placeholder="Opsional..." <?= $isBatal ? 'disabled' : '' ?>>
                </div>
                <div class="bg-slate-50 rounded-xl p-4 text-right">
                    <div class="text-sm text-slate-500">Total</div>
                    <div id="totalLabel" class="text-2xl font-bold text-emerald-600"><?= rupiah($tx['total']) ?></div>
                </div>
            </div>

            <div class="flex justify-end gap-2">
                <a href="<?= url('penjualan', ['action' => 'show', 'id' => $tx['id']]) ?>" class="btn btn-secondary">Batal</a>
                <?php if (!$isBatal): ?>
                    <button type="submit" class="btn btn-primary"><?= icon('check', 'w-4 h-4') ?> Simpan Perubahan</button>
                <?php endif; ?>
            </div>
        </form>
    </div>
...
(function () {
    ...
    var EXISTING = <?= json_encode(array_map(function($d){return ['product_id'=>(int)$d['product_id'],'qty'=>(int)$d['qty'],'harga'=>(float)$d['harga'],'diskon'=>(float)$d['diskon']];}, $details)) ?>;
    var IS_BATAL = <?= $isBatal ? 'true' : 'false' ?>;
    ...
    function formatRibuan(el){
        ...
        el.value = Number(d).toLocaleString('id-ID');
    }
    ...
    function addRow(pre) {
        ...
        row.innerHTML =
            '<div class="col-span-4"><select name="product_id[]" class="input i-produk text-sm" data-no-cs="1" data-placeholder="- Pilih barang -"' + (IS_BATAL ? ' disabled' : '') + '>' + opts + '</select></div>' +
            '<div class="col-span-1"><input type="text" inputmode="numeric" name="qty[]" class="input i-qty text-sm text-center" value="' + qtyStr + '" placeholder="1"' + (IS_BATAL ? ' disabled' : '') + '></div>' +
            '<div class="col-span-2"><input type="text" name="harga[]" class="input i-harga text-sm text-right" inputmode="numeric" value="' + hargaStr + '" placeholder="Harga"' + (IS_BATAL ? ' disabled' : '') + '></div>' +
            '<div class="col-span-2"><input type="text" name="diskon[]" class="input i-diskon text-sm text-right" inputmode="numeric" value="' + diskonStr + '" placeholder="0"' + (IS_BATAL ? ' disabled' : '') + '></div>' +
            '<div class="col-span-2 i-sub text-sm font-semibold text-right">Rp 0</div>' +
            '<div class="col-span-1 text-right">' + (IS_BATAL ? '' : '<button type="button" class="btn btn-ghost p-1.5 btn-hapus">&times;</button>') + '</div>';
        ...
        if (!IS_BATAL) {
            ...
            var btnHapus = row.querySelector('.btn-hapus');
    ...
    if (EXISTING.length === 0 && !IS_BATAL) addRow({});

    var btnTambah = document.getElementById('btnTambahItem');
    if (btnTambah) btnTambah.addEventListener('click', function () { addRow({}); });

    var selMet = document.getElementById('selectMetode');
    if (selMet) selMet.addEventListener('change', function () {
