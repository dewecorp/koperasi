...
class DashboardController extends Controller
{
    public function index(): void
    {
        ...
        $pemasukanBulan = $fin->totalMasuk($monthStart, $monthEnd, $tahunAjaran);
        $pengeluaranBulan = $fin->totalKeluar($monthStart, $monthEnd, $tahunAjaran);
        $penjualanBulan = (float)$pdo->query(
            'SELECT COALESCE(SUM(total),0) FROM transactions WHERE type="penjualan" AND status="AKTIF" AND tahun_ajaran = "' . $tahunAjaran . '" AND tanggal BETWEEN "' . $monthStart . '" AND "' . $monthEnd . '"'
        )->fetchColumn();
        $pembelianBulan = (float)$pdo->query(
            'SELECT COALESCE(SUM(total),0) FROM transactions WHERE type="pembelian" AND status="AKTIF" AND tahun_ajaran = "' . $tahunAjaran . '" AND tanggal BETWEEN "' . $monthStart . '" AND "' . $monthEnd . '"'
        )->fetchColumn();
        ...
        $piutangTotal = (float)$pdo->query(
            'SELECT COALESCE(SUM(r.total - COALESCE((SELECT SUM(p.nominal) FROM receivable_payments p WHERE p.receivable_id = r.id AND p.status="AKTIF"),0)),0)
             FROM receivables r WHERE r.status="AKTIF" AND r.tahun_ajaran = "' . $tahunAjaran . '"'
        )->fetchColumn();
        ...
        $hpp = (float)$pdo->query(
            'SELECT COALESCE(SUM(td.qty * pr.harga_beli),0)
             ...
             WHERE t.type="penjualan" AND t.status="AKTIF" AND t.tahun_ajaran = "' . $tahunAjaran . '" AND t.tanggal BETWEEN "' . $monthStart . '" AND "' . $monthEnd . '"'
        )->fetchColumn();
        $pengeluaranOperasional = (float)$pdo->query(
            'SELECT COALESCE(SUM(total),0) FROM transactions WHERE type="pengeluaran" AND status="AKTIF" AND tahun_ajaran = "' . $tahunAjaran . '" AND tanggal BETWEEN "' . $monthStart . '" AND "' . $monthEnd . '"'
        )->fetchColumn();
        $pemasukanLainBulan = (float)$pdo->query(
            'SELECT COALESCE(SUM(total),0) FROM transactions WHERE type="pemasukan" AND status="AKTIF" AND tahun_ajaran = "' . $tahunAjaran . '" AND tanggal BETWEEN "' . $monthStart . '" AND "' . $monthEnd . '"'
        )->fetchColumn();
        ...
        for ($i = 11; $i >= 0; $i--) {
            ...
            $seriKeluar[] = $fin->totalKeluar($bStart, $bEnd, $tahunAjaran);
        }
        ...
        for ($i = 6; $i >= 0; $i--) {
            ...
            $seriJual[] = (float)$pdo->query(
                'SELECT COALESCE(SUM(total),0) FROM transactions WHERE type="penjualan" AND status="AKTIF" AND tahun_ajaran = "' . $tahunAjaran . '" AND tanggal = "' . $d . '"'
            )->fetchColumn();
        }
        ...
        $rowsKat = $pdo->query(
            'SELECT COALESCE(c.name, "Tanpa Kategori") AS nama, SUM(td.qty * td.harga) AS total
             ...
             WHERE t.type="penjualan" AND t.status="AKTIF" AND t.tahun_ajaran = "' . $tahunAjaran . '"
        ...
        $recent = $pdo->query(
            'SELECT * FROM (
                ...
                FROM transactions t JOIN users u ON u.id = t.user_id WHERE t.type="penjualan" AND t.status="AKTIF" AND t.tahun_ajaran = "' . $tahunAjaran . '"
                UNION ALL
                SELECT t.tanggal, t.no_transaksi, "Pembelian", COALESCE(t.keterangan, t.no_transaksi), t.total, u.username, t.type
                FROM transactions t JOIN users u ON u.id = t.user_id WHERE t.type="pembelian" AND t.status="AKTIF" AND t.tahun_ajaran = "' . $tahunAjaran . '"
                UNION ALL
                SELECT t.tanggal, t.no_transaksi, "Pemasukan Lain", COALESCE(t.keterangan, t.no_transaksi), t.total, u.username, t.type
                FROM transactions t JOIN users u ON u.id = t.user_id WHERE t.type="pemasukan" AND t.status="AKTIF" AND t.tahun_ajaran = "' . $tahunAjaran . '"
                UNION ALL
                SELECT t.tanggal, t.no_transaksi, "Pengeluaran", COALESCE(t.keterangan, t.no_transaksi), t.total, u.username, t.type
                FROM transactions t JOIN users u ON u.id = t.user_id WHERE t.type="pengeluaran" AND t.status="AKTIF" AND t.tahun_ajaran = "' . $tahunAjaran . '"
                UNION ALL
        ...
        $piutangJatuhTempo = $pdo->query(
            'SELECT r.id, c.name AS pelanggan, r.jatuh_tempo, r.total
             ...
             WHERE r.status="AKTIF" AND r.tahun_ajaran = "' . $tahunAjaran . '" AND r.jatuh_tempo IS NOT NULL AND r.jatuh_tempo <= DATE_ADD(CURDATE(), INTERVAL 7 DAY)
        ...
        $hutangJatuhTempo = $pdo->query(
            'SELECT p.id, s.name AS supplier, p.jatuh_tempo, p.total
             ...
             WHERE p.status="AKTIF" AND p.tahun_ajaran = "' . $tahunAjaran . '" AND p.jatuh_tempo IS NOT NULL AND p.jatuh_tempo <= DATE_ADD(CURDATE(), INTERVAL 7 DAY)
